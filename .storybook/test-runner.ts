 ...
 * семантическая карта компонента. Дифф читается как «padding-top: 12px → 10px» и не зависит
 * от шрифтов, сглаживания и ОС.
 *
 ...
 *
 * Baseline в репозиторий не кладётся — как пиксельные снимки и долг axe: 783 файла слепка
 * не код, а состояние на момент прогона. Первый прогон на чистой копии снимает его сам,
 * дальше сверяется с ним; правку токенов видно в диффе следующего прогона.
 *
 * Пересобрать осознанно: UPDATE_STYLE_BASELINES=1 npm run test-storybook
 */
