...
@Component({
    ...
    template: `
        ...
        <div class="card flex flex-wrap gap-6">
            <p-timeline [value]="events" class="w-full md:!w-20">
                <ng-template pTemplate="content" let-event>
                    {{ event.status }}
                </ng-template>
            </p-timeline>

            <p-timeline [value]="events" class="w-full md:!w-20" align="right">
                <ng-template pTemplate="content" let-event>
                    {{ event.status }}
                </ng-template>
            </p-timeline>

            <p-timeline [value]="events" class="w-full md:!w-20" align="alternate">
                <ng-template pTemplate="content" let-event>
                    {{ event.status }}
                </ng-template>
            </p-timeline>
...
export class AlignmentDoc {
    ...
    code: Code = {
        basic: `<p-timeline [value]="events" class="w-full md:!w-20">
    <ng-template pTemplate="content" let-event>
        {{ event.status }}
    </ng-template>
</p-timeline>
