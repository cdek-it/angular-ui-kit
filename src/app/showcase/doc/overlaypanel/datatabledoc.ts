                    ...
                    <p-table [value]="products" selectionMode="single" [(selection)]="selectedProduct" (onRowSelect)="onRowSelect($event, op)" [paginator]="true" [rows]="5" responsiveLayout="scroll">
                        ...
                        <ng-template pTemplate="body" let-rowData let-product>
                            <tr [pSelectableRow]="rowData">
                                <td>{{ product.name }}</td>
                                <td><img src="https://primefaces.org/cdn/primeng/images/demo/product/{{ product.image }}" [alt]="product.image" class="!w-2 shadow-2" /></td>
                                <td>{{ product.price }}</td>
                            </tr>
                        </ng-template>
                    </p-table>
...
<p-overlayPanel #op [style]="{ width: '450px' }" [showCloseIcon]="true">
    <ng-template pTemplate="content">
        ...
        html: `<div class="card flex flex-col items-center gap-3">
<p-toast />
<p-button
    (onClick)="op.toggle($event)"
    icon="pi pi-search"
