...
@Component({
    selector: 'file-upload-template-cdek-button-demo',
    template: `
        <app-docsectiontext>
            <p>Загрузка файлов через кнопку, на базе собственных template согласно дизайн системе СДЭК</p>
            <p>
                Для корректного отображения кнопки и списков файлов необходимо использовать <i>styleClass="p-fileupload-button"</i>, а также шаблоны <i>header</i>, <i>content</i> и <i>file</i>(в противном случае будут применяться стандартные стили
                для аплоадера)
            </p>
        ...
        <div class="card">
            ...
            <p-toast [life]="3000" />
            <p-fileUpload
                [styleClass]="'p-fileupload-button ' + (isSelectedFilesExist ? 'gap-2' : '')"
                [multiple]="true"
                accept="image/*"
                maxFileSize="1000000"
            ...
            >
                <ng-template pTemplate="header" let-files let-chooseCallback="chooseCallback">
                    <div class="flex flex-row gap-2">
                        <p-button label="Загрузить" [raised]="true" [loading]="isFilesProcessing" (onClick)="onOpenChooseDialog()" />
                        <button type="button" class="p-fileupload-info-button" pTooltip="Укажите дополнительную информацию" tooltipPosition="bottom">
                ...
                <ng-template pTemplate="content">
                    <ng-template [ngIf]="isSelectedFilesExist">
                        ...
                        <p-dataView *ngIf="filesListMode === 'Data-View'" [value]="selectedFiles" #dataViewFiles>
...
export class TemplateCdekButtonDoc {
        ...
        basic: `
            ...
            >
                ...
                <ng-template pTemplate="content">
                    <ng-template [ngIf]="isSelectedFilesExist">
                        <!-- Оставить только один -->
                        <p-dataView [value]="selectedFiles" #dataViewFiles>
                            <ng-template pTemplate="list" let-selectedFiles>
                                <div *ngFor="let data of selectedFiles; let i = index" class="p-fileupload-item-row flex flex-row items-center gap-2" [ngClass]="{ error: data.status === uploadStatus.Error }">
                                    <div class="flex flex-row flex-1 items-center gap-2">
                                        <ng-container [ngSwitch]="data.status">
...
@Component({
    selector: 'file-upload-template-cdek-button-demo',
    templateUrl: './file-upload-template-cdek-button-demo.html',
    standalone: true,
    imports: [FileUploadModule, ButtonModule, CommonModule, ProgressSpinnerModule, OrderListModule, DataViewModule, TooltipModule],
    providers: [MessageService]
})
export class TemplateCdekButtonDoc {
    @ViewChild('fileUploadControl') fileUploadControl!: FileUpload;
