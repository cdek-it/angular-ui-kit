            ...
            <p-confirmDialog #cd>
                <ng-template pTemplate="headless" let-message>
                    <div class="flex flex-col items-center p-5 surface-overlay border-round">
                        <div class="border-circle bg-primary inline-flex justify-center items-center !h-6 !w-6">
                            <i class="pi pi-question text-5xl"></i>
                        </div>
                        <span class="font-bold text-2xl block mb-2 mt-4">{{ message.header }}</span>
                        <p class="mb-0">{{ message.message }}</p>
                        <div class="flex items-center gap-2 mt-4">
                            <button pButton label="Save" (click)="cd.accept()" class="!w-8"></button>
                            <button pButton label="Cancel" (click)="cd.reject()" class="p-button-outlined !w-8 "></button>
                        </div>
                    </div>
                </ng-template>
            </p-confirmDialog>
