        ...
        <div class="card">
            <p-megaMenu [model]="items" [style]="{ 'border-radius': '3rem', display: 'flex' }" styleClass="p-3 surface-0">
                <ng-template pTemplate="start">
                    <svg width="33" height="35" viewBox="0 0 33 35" fill="none" xmlns="http://www.w3.org/2000/svg" class="!h-2">
                ...
                <ng-template pTemplate="item" let-item>
                    <a *ngIf="item.root" pRipple class="flex items-center cursor-pointer px-3 py-2 overflow-hidden relative font-semibold text-lg uppercase" style="border-radius: 2rem">
                        <i [ngClass]="item.icon"></i>
                        <span class="ml-2">{{ item.label }}</span>
                    </a>
                    <a *ngIf="!item.root && !item.image" class="flex items-center p-3 cursor-pointer mb-2 gap-2">
                        <span class="inline-flex items-center justify-center border-circle bg-primary !w-3 !h-3">
                    ...
                    <div *ngIf="item.image" class="flex flex-col items-start gap-3">
