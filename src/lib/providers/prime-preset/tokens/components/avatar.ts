export const avatarCss = ({ dt }: { dt: (token: string) => string }): string => `
  :root {
    ...
    --p-avatar-group-border-color:          ${dt('color.bg.surface.raised.default')};
    --p-avatar-group-offset:                calc(-1 * ${dt('dimension.space.300')});
    --p-avatar-lg-group-offset:             calc(-1 * ${dt('dimension.space.300')});
    --p-avatar-xl-group-offset:             calc(-1 * ${dt('dimension.space.600')});
  }
